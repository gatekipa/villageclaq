...
export async function exportPDF({
  ...
  // Generate QR code data URL for footer (best-effort — skip if generation fails)
  let qrDataUrl: string | null = null;
  try {
    qrDataUrl = await QRCode.toDataURL("https://villageclaq.com?ref=report", {
      width: 120,
      margin: 0,
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    // QR code on the last page only — bottom-right, small and unobtrusive
