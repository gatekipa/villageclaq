...
| **F3-09 (Corrections & Reversals UI)** | **COMPLETE / VERIFIED.** Implemented canonical double-entry correction & reversal mutation hook `src/lib/hooks/use-correct-transaction.ts` wrapping RPC `public.correct_financial_event` with strict client validation (3–1000 char reason length, idempotency request tokens, structured RPC error parser `parseCorrectionRpcError`, and query cache invalidation). Implemented audit reversal modal `src/components/finances/projections/reverse-transaction-dialog.tsx` featuring double-entry notices, original event summary, live character counting, and pending/success state handling. Integrated reversal actions into `src/components/finances/projections/general-ledger-cashbook.tsx` across both the interactive table rows and audit detail sheet footer, strictly guarded with `finances.manage` permissions and multi-condition invariant validation (preventing reversal of reversals, opening balances, or already corrected transactions). Verified with 100% EN/FR translation key parity (52 keys in `transactionCorrections`), clean ESLint/TSC, and passing test suites. |
| **Next active slice** | **F3-10 (Opening Cash & Balance Migration UI)** designated per PRD Section 10. Merge/deploy to `origin/main` remains separately founder-controlled. |
...
5. **F3-09 (Corrections & Reversals UI)** — **COMPLETE / VERIFIED.** Implemented client RPC mutation hook `src/lib/hooks/use-correct-transaction.ts`, reversal dialog `src/components/finances/projections/reverse-transaction-dialog.tsx`, and data table / audit sheet action integration in `src/components/finances/projections/general-ledger-cashbook.tsx`. Guarded with `finances.manage` permission and double-entry invariants. Verified with 100% EN/FR parity (52 keys), clean type checks, and full test suite passes.
6. **Next active slice: F3-10 (Opening Cash & Balance Migration UI)** — Designated as the active work item per Section 10 and Section 26 of the Master Rebuild PRD (`docs/VILLAGECLAQ_MASTER_REBUILD_PRD_V1.md`).
7. **Production migration release gate** — Production application remains separately founder-authorized after completion of F3-06…F3-10.
...
|-------|-------|
| Owner | Jude Anyere |
| Next bounded action | Advance to **F3-10 (Opening Cash & Balance Migration UI)** per Master PRD sequence. |
| Permitted scope | F3-10 implementation. No direct mutation of production. |
| Remaining acceptance | F3-10 opening balance posting and epoch-anchored migration verification contracts. |
...
Next Slice: `F3-10 (Opening Cash & Balance Migration UI)`
