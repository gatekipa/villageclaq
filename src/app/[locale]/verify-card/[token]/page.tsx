...
export default function VerifyCardPage({ params }: { params: { token: string } }) {
  ...
  if (isLoading) return <div className="p-8 text-center">Loading verification...</div>;
  if (error || !data || !data.valid) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white p-6 rounded-lg shadow max-w-sm w-full text-center space-y-4">
          <h1 className="text-xl font-bold text-gray-800">Card Not Found or Revoked</h1>
          <p className="text-sm text-gray-500">This community card is invalid or no longer active.</p>
        </div>
      </div>
    );
  }

  const sanitized = sanitizePublicCardPayload(data);
  if (!sanitized) {
    return <div className="p-8 text-center text-red-500">Data integrity error</div>;
  }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full space-y-4 border-t-4 border-primary">
        ...
        <div className="space-y-1">
          <p className="text-sm text-gray-500 uppercase tracking-wide">Organization</p>
          <p className="font-medium text-gray-900">{sanitized.organizationName}</p>
        </div>
        
        <div className="space-y-1">
          <p className="text-sm text-gray-500 uppercase tracking-wide">Member</p>
          <p className="font-medium text-gray-900">{sanitized.memberDisplayName}</p>
        </div>

        <div className="space-y-1">
          <p className="text-sm text-gray-500 uppercase tracking-wide">Issued On</p>
          <p className="text-sm text-gray-900">{new Date(sanitized.issuedAt).toLocaleDateString()}</p>
        </div>
