...
const db = createDisposablePostgres({ name: "f3-foundation", label: "f3-foundation" });
...
const seedActors = `
  ...
  INSERT INTO public.groups(id,organization_id,name,currency) VALUES
  ...
  INSERT INTO public.projects(id,group_id,name) VALUES
    ('${projectA}','${groupA}','A project'),('${projectB}','${groupB}','B project');
  INSERT INTO public.financial_ledger_epochs
    (id,group_id,currency,effective_from,effective_to,source_kind,approval_note) VALUES
    ('${epochA}','${groupA}','USD','2026-01-01',NULL,'cutover','Disposable foundation fixture'),
    ('${epochAOld}','${groupA}','USD','2025-01-01','2026-01-01','cutover','Disposable foundation fixture'),
    ('${epochAXaf}','${groupA}','XAF','2024-01-01','2025-01-01','cutover','Disposable foundation fixture'),
    ('${epochB}','${groupB}','USD','2026-01-01',NULL,'cutover','Disposable foundation fixture'),
    ('${epochXaf}','${groupXaf}','XAF','2026-01-01',NULL,'cutover','Disposable foundation fixture'),
    ('${epochInternal}','${internalGroup}','USD','2026-01-01',NULL,'cutover','Disposable foundation fixture');
  INSERT INTO financial_private.internal_financial_tenants(organization_id,tenant_kind)
    VALUES('${internalOrg}','qa');
`;
...
before(async () => {
  await db.start();
  installF3StubFloor(db.url, { through: "00118_f3_bounded_financial_epoch_foundation.sql" });
  sql(seedActors);
  applyForwardMigration(db.url, "00119_f3_01_core_ledger_foundation.sql");
  sql(seedConfig);
});
