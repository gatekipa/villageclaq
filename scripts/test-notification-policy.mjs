...
function enabledHostingPolicy(overrides = {}) {
  return {
    ...DEFAULT_HOSTING_POLICY,
    enabled: true,
...
// ── Defaults (dormant-by-default) ─────────────────────────────────────────

test("hosting default is 7 days before, maxOccurrences 1, no repeat, dormant", () => {
  ...
  assert.equal(DEFAULT_HOSTING_POLICY.channels.push, false);
  assert.deepEqual(DEFAULT_HOSTING_POLICY.channels, DORMANT_CHANNELS);
  assert.equal(validatePolicyConfig(DEFAULT_HOSTING_POLICY).ok, true);
});

test("event default is 48 hours before, maxOccurrences 1, dormant", () => {
  assert.equal(DEFAULT_EVENT_POLICY.triggers[0].offsetHours, -48);
  assert.equal(DEFAULT_EVENT_POLICY.maxOccurrences, 1);
  assert.equal(DEFAULT_EVENT_POLICY.enabled, false);
  assert.equal(DEFAULT_EVENT_POLICY.channels.push, false);
...
test("payment compat contract docs object exists and does not pretend +24h live", () => {
  ...
  assert.equal(FUTURE_PAYMENT_POLICY.enabled, false);
  assert.equal(FUTURE_PAYMENT_POLICY.channels.push, false);
...
test("M2 domains are payment|hosting|event; announcement is Cut2 type not policy domain", () => {
  assert.deepEqual([...M2_POLICY_DOMAINS], ["payment", "hosting", "event"]);
  assert.equal(ANNOUNCEMENT_IS_CUT2_TYPE_NOT_M2_POLICY_DOMAIN, true);
  assert.equal(POLICY_DISPOSITION_BOUNDARY.enqueueEligibleKind, "ENQUEUE_ELIGIBLE");
  assert.equal(POLICY_DISPOSITION_BOUNDARY.retiredSendNowKind, "SEND_NOW");
});
...
test("invalid timezone rejected (never silent ENQUEUE_ELIGIBLE)", () => {
...
test("push true is rejected (Cut 2 DENY; no push eligibility)", () => {
  const r = validatePolicyConfig(
    basePolicy({
...
test("hosting + email true is rejected by Cut 2 floor", () => {
  const r = validatePolicyConfig(
    enabledHostingPolicy({
      channels: { in_app: true, email: true, sms: true, whatsapp: true, push: false },
    }),
  );
  assert.equal(r.ok, false);
...
test("disabled / dormant default policy stops with STOP_POLICY", () => {
  const d = evaluateDisposition({
...
test("push is hard DENY even when evaluating the push channel", () => {
  const d = evaluateDisposition({
    policy: basePolicy(),
    now: new Date("2026-09-10T12:00:00Z"),
    resolved: false,
...
test("quiet hours defer instead of enqueue-eligible", () => {
  const policy = basePolicy({
    timezone: "UTC",
...
test("ENQUEUE_ELIGIBLE outside quiet hours (trusted-enqueue semantics, not send)", () => {
  const policy = basePolicy({
    timezone: "UTC",
    quietHours: { startMinute: 22 * 60, endMinute: 7 * 60 },
  });
...
test("same-day quiet window", () => {
  const quiet = { startMinute: 12 * 60, endMinute: 14 * 60 };
...
test("evaluator module has no send/queue/provider side effects in this process", () => {
  const before = process.env.PAYMENT_REMINDER_CONFIRMED_BASIS;
  const d = evaluateDisposition({
    policy: basePolicy(),
    now: new Date("2026-09-10T12:00:00Z"),
    resolved: false,
  ...
  assert.equal(d.kind, "ENQUEUE_ELIGIBLE");
  assert.equal(process.env.PAYMENT_REMINDER_CONFIRMED_BASIS, before);
});
