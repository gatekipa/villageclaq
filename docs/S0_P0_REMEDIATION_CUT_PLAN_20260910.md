...
**Date:** 2026-09-10  
**Status:** PLANNING FREEZE — DO NOT IMPLEMENT  
**Overall recommended verdict:** **HOLD**  
**Daybreak handoff:** **HOLD** (see §21)  
...
S0-C PASS ──┘         │
                      ├── HOLD if live helper-caller inventory incomplete
                      │         or duplicate permissive policies neutralize helpers
                      ▼
...
**Soft edges (do not block Cut 1 contract freeze, do block Daybreak implement)**

- Live `pg_policies` / `pg_proc` inventory of every helper caller (S0-A sampled; not exhaustive).
- Duplicate permissive policies that OR with helper-based policies (00109 class).
- Inline `memberships` EXISTS clauses that never call the five helpers.
...
| `rls_evote_insert` (`00026`) | INSERT | **ACTIVE-ONLY OPERATIONAL AUTHORITY** (legacy `election_votes`) | **UNKNOWN** if table/policy still live after 00073 ballot model — fail-closed |
...
| `get_relief_branch_summary` (`00111`, **applied**) | RPC/view | Org-boundary privileged aggregate | **ACTIVE-ONLY PRIVILEGED READ** — migrate function bound to `get_user_active_group_ids` **or** add active filter inside the DEFINER. **UNKNOWN** whether HQ oversight requires inactive membership — fail-closed for ops; rehearsal must qualify HQ |
| Proxy INSERT (S0-A highlight; policy name **not** given in S0-A) | INSERT | **ACTIVE-ONLY OPERATIONAL** | **UNKNOWN live policy name** — **HOLD blocker** |
...
|--------|----------|----------------|-------|
| `create_proxy_member` (`00015`; `00103` hardening **not** in prod log) | EXISTS any membership for caller; **no status, no admin/permission** | **ACTIVE-ONLY OPERATIONAL AUTHORITY** | **REPLACE** in Cut 1: require `is_active_group_member` **and** (`is_group_admin` or `has_group_permission(..., 'members.manage')`) after those helpers are active-gated |
...
| Inline `EXISTS (memberships … role IN ('owner','admin'))` on `loan_configs_*`, `loans_*`, `fine_types_admin`, `00001` memberships/positions policies, `00030` committee ALL | No helper, no status | **ACTIVE-ONLY OPERATIONAL** if live | **Cannot close via helper REPLACE.** Live inventory required. **HOLD** if still present (duplicate permissive OR) |
| `"Group owners/admins can update memberships"` (`00001`) **OR** `rls_membership_role_guard` (`00026`) | Classic 00109 neutralization | **UNKNOWN** if both live | **HOLD blocker** until `pg_policies` for `memberships` is dumped on disposable restore |

### 5.7 HOLD blockers from this matrix

1. **No exhaustive live `pg_policies` dump in S0-A** keyed by helper name. Repo journal + applied 00072+ replacements are a **sample**, not proof of the live set.
2. **Duplicate permissive policies** can neutralize helper tightening (proven class: 00109 vs 00108). Memberships UPDATE / loans / fines / committees are suspects.
3. **Proxy INSERT policy name** using `get_user_group_ids` is asserted by S0-A but **not named** in S0-A.
4. **`get_relief_branch_summary` HQ path** — active-only vs org historical read is **UNKNOWN**.
5. **`rls_evote_insert` / `election_votes`** — UNKNOWN whether still live after 00073.

Until a disposable rehearsal dumps live callers and classifies every **WRITE / RPC / privileged-read** hit, Daybreak must **HOLD**.
...
| `public` | `create_proxy_member` | live signature from `00015` / prod catalog (rehearsal must `pg_get_functiondef`; `00103` overload **not** live) |
...
`get_relief_branch_summary`: **TOUCH only if** rehearsal classifies it ACTIVE-ONLY PRIVILEGED READ and names this function in the allowlist; otherwise **UNKNOWN — leave** and HOLD that sub-path.

### 7.4 Policies — REWRITE in Cut 1 (is_group_member WRITE → `is_active_group_member`)

Only if rehearsal confirms the policy still exists **and** still calls `is_group_member`:
...
| `rls_evote_insert` | `election_votes` | INSERT (skip if relation/policy absent) |

**Do not rewrite** SELECT policies that only use `is_group_member` for visibility.

### 7.5 Policies — REWRITE in Cut 1 (`get_user_group_ids` WRITE / privileged)

Only if rehearsal confirms:

| Policy / object | Table / fn | Why |
|-----------------|------------|-----|
| S0-A “Proxy INSERT gated by `get_user_group_ids`” | `memberships` (name **UNKNOWN** until dump) | **HOLD** until named |
| `"Users can create transfers"` | `sub_group_transfers` | INSERT |
| `"Admins can manage committees"` / `"Admins can manage committee members"` / `"Admins can update transfers"` / member_transfers admin writes (`00030`) | listed tables | ALL/UPDATE/INSERT — also have **inline role** checks that must be active-gated if live |
| audit_log INSERT using `get_user_group_ids` (`00035`) | `group_audit_logs` or successor name | INSERT |
| `get_relief_branch_summary` | function | privileged read — only if qualified |

### 7.6 Policies — inherit REPLACE (no DROP/CREATE in Cut 1)

Named so testers know they change **without** a policy rewrite:

`mm_insert`, `mm_update`, `mm_delete`, `ann_insert`, `ann_update`, `ann_delete`, manager branch of `ann_select` / `mm_select`, `pct_manage`, `rls_notif_insert`, `rls_co_insert`, `rls_co_update`, `ec_manage_insert`, `ec_manage_update`, `Group owners/admins can update` (`groups`), `Members can leave or admins can remove members` (admin branch), `Group admins can delete payments`, `Group admins can update payments`, `poa_select_admin`, `rls_membership_role_guard` (if live), plus any live `00026`/`00048`/`00037`/`00060` policy whose only gate is `is_group_admin` / `has_group_permission` / `is_group_admin_or_owner`.

If rehearsal finds a **second** permissive policy on the same table/command that inlines status-blind membership, Cut 1 **must either** DROP/replace that named policy **or** HOLD. Suspected names (not confirmed live):

- `"Group owners/admins can update memberships"`
- `"Group owners/admins can manage memberships"`
- `"Group owners/admins can manage positions"`
- `"Group owners/admins can manage assignments"`
- `loan_configs_insert`, `loan_configs_update`, `loan_configs_delete`
- `loans_insert`, `loans_update`, `loans_delete`
- `fine_types_admin`
...
Daybreak may **implement Cut 1 only after this plan is PASS**.  
This planning artifact's recommended verdict is **HOLD**.
...
| Exact object scope (functions) | PASS (named) |
| Exact object scope (policies) | **HOLD** — live dump missing; proxy INSERT unnamed; duplicate permissive suspects |
| Critical operational callers classified | **HOLD** — §5.7 |
| F3 never weaker | PASS (design) |
| Historical access not invented | PASS |
| No implement / no prod apply in this PR | PASS |

**Daybreak handoff: HOLD.**

Implement is forbidden until a follow-up planning delta (or rehearsal evidence addendum) clears §5.7 with named live policies and either includes them in §7 or explicitly marks them NOT IN CUT SCOPE with a residual-risk accept.

PASS criteria for a future handoff flip (docs-only or evidence-only; still no implement in that flip unless separately authorized):

1. Disposable dump of helper-matching `pg_policies` + `pg_proc`.
2. Every WRITE/RPC/privileged-read extra is named and classified.
3. Duplicate permissive memberships/loans/fines/committees policies confirmed absent **or** added to §7.6 rewrite list.
4. Proxy INSERT policy name recorded.
5. `get_relief_branch_summary` HQ path classified.
...
14. Daybreak starting implementation while this document still says **HOLD**.
...
|------|-------|
| Overall recommended verdict | **HOLD** |
| Daybreak implement Cut 1 | **HOLD** |
| Why not PASS | Incomplete live helper-caller inventory; unnamed proxy INSERT policy; possible duplicate permissive policies that neutralize helper REPLACE; `get_relief_branch_summary` HQ classification UNKNOWN |
| What is frozen anyway | Pins; P0 trio; order Cut1→2→3; active-only operational helper contract; F3 compatibility; migration rules; Cut 2/3 bounded contracts; STOP rules |
| Prod apply | ZERO |
| Historical migrations modified | NO |
| Mutations / messages | ZERO |

**Next authorized step (not automatic):** disposable live `pg_policies`/`pg_proc` inventory (S0-B isolated restore or S0-C baseline pack) → planning addendum → Daybreak PASS/HOLD flip → only then Cut 1 migration authorship.
