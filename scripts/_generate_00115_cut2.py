#!/usr/bin/env python3
"""Assemble 00115 from frozen WA index matrix. Not a CI script."""
...
def fingerprint_index(name: str, expected: str, abort_label: str) -> str:
    return f"""
  ...
  IF v_indexdef IS NULL THEN
    RAISE EXCEPTION 'CUT2_ABORT: missing {abort_label} {name}';
  END IF;
  IF v_indexdef IS DISTINCT FROM '{sql_str(expected)}' THEN
    RAISE EXCEPTION 'CUT2_ABORT: {abort_label} indexdef drift for {name}';
  END IF;"""


wa_fp = "\n".join(fingerprint_index(i["index_name"], i["old_exact_indexdef"], "WhatsApp unique") for i in indexes)
other_fp = "\n".join(fingerprint_index(n, d, "reviewed queue") for n, d in other)
wa_rebuild = "\n".join(
    f"  DROP INDEX public.{i['index_name']};\n  {i['future_exact_indexdef']};" for i in indexes
)
def pg16_normalized_future(i: dict) -> str:
    """PostgreSQL 16 flattens ((<old body>) AND cut2_provenance_version = 1)
    into (<old body> AND (cut2_provenance_version = 1)). CREATE still uses
    the frozen future_exact_indexdef verbatim; this is the stored indexdef."""
    prefix = i["old_exact_indexdef"].split(" WHERE ", 1)[0]
    return (
        prefix
        + " WHERE ("
        + i["old_where_body_without_outer_parens"]
        + " AND (cut2_provenance_version = 1))"
    )


def fingerprint_rewritten(i: dict) -> str:
    name = i["index_name"]
    planned = sql_str(i["future_exact_indexdef"])
    stored = sql_str(pg16_normalized_future(i))
    return f"""
  SELECT indexdef INTO v_indexdef
    FROM pg_catalog.pg_indexes
   WHERE schemaname = 'public'
     ...
     AND indexname = '{name}';
  IF v_indexdef IS NULL THEN
    RAISE EXCEPTION 'CUT2_ABORT_POST: missing rewritten WhatsApp unique {name}';
  END IF;
  IF v_indexdef IS DISTINCT FROM '{planned}'
     AND v_indexdef IS DISTINCT FROM '{stored}' THEN
    RAISE EXCEPTION 'CUT2_ABORT_POST: rewritten WhatsApp unique indexdef drift for {name}: %', v_indexdef;
  END IF;
  IF position('cut2_provenance_version = 1' in v_indexdef) = 0 THEN
    RAISE EXCEPTION 'CUT2_ABORT_POST: rewritten WhatsApp unique {name} is not provenance-scoped';
  END IF;"""


wa_post = "\n".join(fingerprint_rewritten(i) for i in indexes)
...
header = f"""-- S0 Cut 2 — P0-B notification queue trusted-enqueue boundary
--
...
out = Path("/workspace/supabase/migrations/00115_s0_p0b_cut2_notification_queue.sql")
assembled = (
    header
    .replace("{other_fp}", other_fp)
    .replace("{wa_fp}", wa_fp)
    .replace("{wa_rebuild}", wa_rebuild)
    .replace("{rpc}", rpc)
    .replace("{wa_post}", wa_post)
)
