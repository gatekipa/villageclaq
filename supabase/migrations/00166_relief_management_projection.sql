-- R-010: read-only management elimination of reciprocal Relief agency
-- balances. Every local F3 posting remains untouched and authoritative.
-- An incomplete branch/owner pair remains visible but is not eliminated.
BEGIN;
...
BEGIN
  ...
  WITH authorized AS (
    ...
    SELECT a.plan_id,r.branch_group_id,a.financial_owner_group_id,
      a.topology_version,r.branch_event_id,r.owner_event_id
    ...
    WHERE r.status='confirmed'
      AND r.branch_event_id IS NOT NULL AND r.owner_event_id IS NOT NULL
  ), in_perimeter AS (
    ...
    SELECT linked.plan_id,linked.branch_group_id,
      ...
      coalesce(branch_side.amount,0) branch_signed,
  ...
  SELECT balances.plan_id,balances.branch_group_id,
    balances.financial_owner_group_id,p_currency,balances.topology_version,
    p_as_of,balances.branch_due,balances.owner_receivable,
    greatest(least(balances.branch_due,balances.owner_receivable),0),
    balances.branch_due-greatest(least(
      balances.branch_due,balances.owner_receivable),0),
    balances.owner_receivable-greatest(least(
      balances.branch_due,balances.owner_receivable),0)
  FROM balances;
END
$$;
