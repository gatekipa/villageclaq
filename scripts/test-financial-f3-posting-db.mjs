// Isolated PostgreSQL 17 only. No production URL. Forward 00118–00120 only.
...
const asyncSql = (query) => db.asyncSql(query);
const migrationChain = F3_FORWARD_CHAIN.slice(0, 3);
...
function seed(context = fixtureContext()) {
  ...
  for (const g of [ID.group,ID.otherGroup,ID.xafGroup]) {
    ...
    query += "INSERT INTO public.group_positions(id,group_id,title) VALUES("+q(uuid(9200+Number(g.slice(-1))))+","+q(g)+",'Treasurer');";
...
before(async () => {
  await db.start();
  installF3StubFloor(db.url, { through: "00120_f3_02_secure_posting_idempotency.sql" });
});
after(() => {
  db.stop();
});
test("A. prerequisite chain and public posting smoke", () => {
  ...
  assert.equal(migrationChain.length,3);
  seed();
...
function holdIdentity(input) {
  ...
  const ready = new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code !== 0 && code !== null) {
        /* holder ends via COMMIT/ROLLBACK */
      }
    });
