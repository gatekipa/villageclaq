    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white p-6 rounded-lg shadow-sm max-w-sm w-full text-center space-y-4 border border-gray-100">
          <h1 className="text-xl font-bold text-gray-800">{t('notFound')}</h1>
          <p className="text-sm text-gray-500">{t('notFoundDetail')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <footer className="p-6 text-center border-t bg-white mt-auto">
        <ProfileAbuseReport slug={profile.slug} />
        <Link href="/org" className="inline-flex min-h-11 items-center px-3 text-sm text-gray-600 hover:underline">
          {t('directoryTitle')}
        </Link>
        <a 
          href="/" 
