...
-- T1_BEGIN
-- Isolation is READ COMMITTED: SERIALIZABLE snapshot is taken at the first
-- query (T1 observation / advisory DO) BEFORE T2 relation locks, so T3 would
-- miss dependency drift committed while reset waits. ACCESS EXCLUSIVE on
-- leftover relations + SHARE ROW EXCLUSIVE on schema_migrations + T3 exact
-- tuple revalidation retain the previous protections.
BEGIN ISOLATION LEVEL READ COMMITTED;
SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '60s';
SET LOCAL idle_in_transaction_session_timeout = '30s';
SET LOCAL application_name = 'f13_qualification_reset';
DO $f15_advisory_xact_lock$
BEGIN
  PERFORM pg_advisory_xact_lock(-1953389924, -753430976);
END
$f15_advisory_xact_lock$;
-- T1 backend identity is emitted from THIS same connection
-- executing generated reset SQL. Application name alone is not identity.
DO $f18_backend_ident$
BEGIN
  RAISE NOTICE 'F18_RESET_BACKEND pid=% datname=% app=%', pg_backend_pid(), current_database(), current_setting('application_name', true);
END
$f18_backend_ident$;
SELECT json_build_object(
  'schema', 'f15-qualification-reset-tx-observation-v1',
  'phase', 'T1_BEGIN',
  'event', 'began',
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'F18_RESET_LOCK_ACQUIRED pid=%', pg_backend_pid();
END
$f13_lock$;
SELECT json_build_object(
  'schema', 'f15-qualification-reset-tx-observation-v1',
...
BEGIN
  ...
  IF extra IS NOT NULL THEN
    RAISE EXCEPTION 'F13_UNEXPECTED_OBJECT_OR_DEPENDENCY: %', extra;
  END IF;
  -- T3 live catalog dependency tuples after lock. Not a JS-capture substitute.
  extra := NULL;
  SELECT format('foreign_key|%s|%s|%s', con.conname, n.nspname || '.' || rel.relname, fn.nspname || '.' || frel.relname) INTO extra
  FROM pg_constraint con
  JOIN pg_class rel ON rel.oid = con.conrelid
  JOIN pg_namespace n ON n.oid = rel.relnamespace
  JOIN pg_class frel ON frel.oid = con.confrelid
  ...
  WHERE con.contype = 'f'
    ...
    AND format('foreign_key|%s|%s|%s', con.conname, n.nspname || '.' || rel.relname, fn.nspname || '.' || frel.relname) <> ALL (ARRAY['foreign_key|memberships_group_id_fkey|public.memberships|public.groups', 'foreign_key|projects_group_id_fkey|public.projects|public.groups', 'foreign_key|notification_policies_group_id_fkey|public.notification_policies|public.groups', 'foreign_key|notification_policy_occurrences_group_id_fkey|public.notification_policy_occurrences|public.groups', 'foreign_key|position_assignments_membership_id_fkey|public.position_assignments|public.memberships', 'foreign_key|notification_policy_triggers_policy_id_fkey|public.notification_policy_triggers|public.notification_policies', 'foreign_key|financial_ledger_epochs_group_id_fkey|public.financial_ledger_epochs|public.groups', 'foreign_key|financial_ledger_epochs_created_by_fkey|public.financial_ledger_epochs|public.profiles', 'foreign_key|financial_ledger_epochs_approved_by_fkey|public.financial_ledger_epochs|public.profiles', 'foreign_key|internal_financial_tenants_organization_id_fkey|financial_private.internal_financial_tenants|public.organizations', 'foreign_key|internal_financial_tenants_created_by_fkey|financial_private.internal_financial_tenants|public.profiles', 'foreign_key|financial_accounts_group_id_fkey|public.financial_accounts|public.groups', 'foreign_key|financial_accounts_created_by_fkey|public.financial_accounts|public.profiles', 'foreign_key|financial_accounts_epoch_scope|public.financial_accounts|public.financial_ledger_epochs', 'foreign_key|financial_funds_group_id_fkey|public.financial_funds|public.groups', 'foreign_key|financial_funds_created_by_fkey|public.financial_funds|public.profiles', 'foreign_key|financial_categories_group_id_fkey|public.financial_categories|public.groups', 'foreign_key|financial_categories_created_by_fkey|public.financial_categories|public.profiles', 'foreign_key|financial_events_group_id_fkey|public.financial_events|public.groups', 'foreign_key|financial_events_created_by_fkey|public.financial_events|public.profiles', 'foreign_key|financial_events_epoch_scope|public.financial_events|public.financial_ledger_epochs', 'foreign_key|financial_events_reversal_scope|public.financial_events|public.financial_events', 'foreign_key|financial_events_replacement_scope|public.financial_events|public.financial_events', 'foreign_key|financial_postings_event_scope|public.financial_postings|public.financial_events', 'foreign_key|financial_postings_account_scope|public.financial_postings|public.financial_accounts', 'foreign_key|financial_postings_fund_scope|public.financial_postings|public.financial_funds', 'foreign_key|financial_postings_category_scope|public.financial_postings|public.financial_categories', 'foreign_key|financial_postings_member_scope|public.financial_postings|public.memberships', 'foreign_key|financial_postings_project_scope|public.financial_postings|public.projects', 'foreign_key|posting_command_payloads_event_id_fkey|financial_core.posting_command_payloads|public.financial_events', 'foreign_key|correction_command_payloads_group_id_fkey|financial_core.correction_command_payloads|public.groups', 'foreign_key|correction_command_payloads_correction_actor_fkey|financial_core.correction_command_payloads|public.profiles', 'foreign_key|correction_command_payloads_target_event_id_group_id_fkey|financial_core.correction_command_payloads|public.financial_events', 'foreign_key|correction_command_payloads_reversal_event_id_group_id_fkey|financial_core.correction_command_payloads|public.financial_events', 'foreign_key|correction_command_payloads_replacement_event_id_group_id_fkey|financial_core.correction_command_payloads|public.financial_events', 'foreign_key|opening_provenances_group_id_fkey|financial_core.opening_provenances|public.groups', 'foreign_key|opening_provenances_recorded_by_fkey|financial_core.opening_provenances|public.profiles'])
  LIMIT 1;
  IF extra IS NOT NULL THEN
    RAISE EXCEPTION 'F13_UNEXPECTED_OBJECT_OR_DEPENDENCY: live tuple not in FINITE_DEPENDENCY_ALLOWLIST %', extra;
  END IF;
  extra := NULL;
  SELECT format('foreign_key|%s|%s|%s', con.conname, n.nspname || '.' || rel.relname, fn.nspname || '.' || frel.relname) INTO extra
  FROM pg_constraint con
  ...
  WHERE con.contype = 'f'
    AND n.nspname IN ('public', 'financial_core', 'financial_private')
    AND format('foreign_key|%s|%s|%s', con.conname, n.nspname || '.' || rel.relname, fn.nspname || '.' || frel.relname) <> ALL (ARRAY['foreign_key|memberships_group_id_fkey|public.memberships|public.groups'])
  LIMIT 1;
  IF extra IS NOT NULL THEN
    RAISE EXCEPTION 'F13_UNEXPECTED_OBJECT_OR_DEPENDENCY: live tuple not in captured approved starting state %', extra;
  END IF;
  FOREACH expected_key IN ARRAY ARRAY['foreign_key|memberships_group_id_fkey|public.memberships|public.groups']
  LOOP
    extra := NULL;
    SELECT format('foreign_key|%s|%s|%s', con.conname, n.nspname || '.' || rel.relname, fn.nspname || '.' || frel.relname) INTO extra
    FROM pg_constraint con
  ...
  WHERE con.contype = 'f'
    AND n.nspname IN ('public', 'financial_core', 'financial_private')
      AND format('foreign_key|%s|%s|%s', con.conname, n.nspname || '.' || rel.relname, fn.nspname || '.' || frel.relname) = expected_key
    LIMIT 1;
    IF extra IS NULL THEN
      RAISE EXCEPTION 'F13_UNEXPECTED_OBJECT_OR_DEPENDENCY: missing captured approved dependency %', expected_key;
    END IF;
  END LOOP;
