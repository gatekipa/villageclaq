...
test("perf staleTime added without dropping the confirmed-only money basis", () => {
  ...
  assert.ok(/select\("group_id, currency, ledger_epoch_id, id, amount, status, obligation_id, relief_plan_id, membership_id, contribution_type_id"\)/.test(hooks), "dashboard stats retain currency, epoch, status, relief exclusion and allocation identity");
