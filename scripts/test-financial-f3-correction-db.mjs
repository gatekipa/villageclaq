...
import{installPrerequisites}from"./fixtures/financial-f3-posting-prerequisites.mjs";
...
before(async()=>{await db.start();
 installPrerequisites(sql);sql("ALTER TABLE public.profiles ADD COLUMN display_name text,ADD COLUMN full_name text;");sql(read("supabase/migrations/20260909022633_f3_projection_read_proof.sql"));sql(read("supabase/migrations/20260909054500_f3_correction_reversal.sql"));});
after(()=>db.stop());
