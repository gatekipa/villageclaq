...
// 1. Inject cleanly into page.tsx
...
if (!pageContent.includes('CanonicalReportRenderer')) {
  ...
  const importIndex = lines.findIndex(l => l.includes('import { getMemberName } from "@/lib/get-member-name";'));
  lines.splice(importIndex + 1, 0, 'import { CanonicalReportRenderer } from "./canonical-statement";');
}

// Find the return statement inside ReportDetailContent (which is around line 1307 after restoring)
// The first one is around line 218 inside ReportDetailPage, we want the second one.
let returnCount = 0;
...
if (injectionIndex > -1 && !pageContent.includes('CanonicalReportRenderer reportId={reportId}')) {
  lines.splice(injectionIndex, 0, earlyReturnBlock);
}
fs.writeFileSync(pagePath, lines.join('\n'));

// 2. Rewrite canonical-statement.tsx precisely
