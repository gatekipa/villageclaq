...
import { F3_FORWARD_CHAIN, installF3StubFloor } from "./fixtures/f3-forward-prerequisites.mjs";
...
const db = createDisposablePostgres({ name: label, label });
const migrationChain = F3_FORWARD_CHAIN.slice(0, 4);
...
function seedVector(source, queryGroup) {
  ...
  setup += `INSERT INTO public.memberships(id,group_id,user_id,display_name) VALUES
    ...
    INSERT INTO public.group_positions(id,group_id,title) VALUES
      (${q(positionView)},${q(queryGroup)},'Auditor'),
      (${q(positionManage)},${q(queryGroup)},'Treasurer'),
      (${q(positionOther)},${q(otherGroup)},'Auditor');
...
before(async () => {
  await db.start();
  installF3StubFloor(db.url, { through: "00121_f3_03_projection_read_proof.sql" });
});
after(() => {
  db.stop();
});
...
test("Y. catalog, grants, migration order, and representative plans", () => {
  seedVector(foundation(), ID.group);
  assert.equal(migrationChain.length, 4);
  assert.equal(sql(`SELECT string_agg(name,',' ORDER BY name) FROM (VALUES
    ('00118_f3_bounded'),('00119_f3_01_core'),('00120_f3_02_secur'),('00121_f3_03_proje')) v(name);`),
  "00118_f3_bounded,00119_f3_01_core,00120_f3_02_secur,00121_f3_03_proje");
