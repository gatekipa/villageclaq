...
export function paymentReceiptEmail(data: PaymentReceiptData, locale: "en" | "fr" = "en"): string {
  ...
  const body = `
    ...
    <p style="margin:0 0 20px; font-size:15px; color:#475569;">
      ${isEn
        ? `Hi ${data.memberName}, your payment has been recorded for <strong>${data.groupName}</strong>.`
        : `Bonjour ${data.memberName}, votre paiement a été enregistré pour <strong>${data.groupName}</strong>.`}
    </p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc; border-radius:8px; padding:16px; margin:0 0 20px;">
      <tr>
        ...
        <td style="padding:8px 16px; font-size:16px; font-weight:700; color:#1db981; text-align:right;">${data.amount}</td>
      </tr>
      <tr>
        <td style="padding:8px 16px; font-size:13px; color:#64748b;">${isEn ? "Type" : "Type"}</td>
        <td style="padding:8px 16px; font-size:13px; font-weight:600; text-align:right;">${data.contributionType}</td>
      </tr>
      <tr>
        <td style="padding:8px 16px; font-size:13px; color:#64748b;">${isEn ? "Method" : "Méthode"}</td>
        <td style="padding:8px 16px; font-size:13px; text-align:right;">${method}</td>
      </tr>
      <tr>
        <td style="padding:8px 16px; font-size:13px; color:#64748b;">${isEn ? "Date" : "Date"}</td>
        <td style="padding:8px 16px; font-size:13px; text-align:right;">${data.date}</td>
      </tr>
      ${data.reference ? `<tr>
        <td style="padding:8px 16px; font-size:13px; color:#64748b;">${isEn ? "Reference" : "Référence"}</td>
        <td style="padding:8px 16px; font-size:13px; text-align:right;">${data.reference}</td>
      </tr>` : ""}
      <tr>
        <td style="padding:8px 16px; font-size:13px; color:#64748b;">${isEn ? "Recorded By" : "Enregistré par"}</td>
        <td style="padding:8px 16px; font-size:13px; text-align:right;">${data.recordedBy}</td>
      </tr>
    </table>
